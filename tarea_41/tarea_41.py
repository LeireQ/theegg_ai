...
text1 = """En Strandhill, Irlanda, se cruzó en mi camino Chris, un señor de los que inspiran y se posicionan como
...
def count_characters():
    pattern= r"\s+" #Matches Unicode whitespace characters [ \t\n\r\f\v] 
    characters = len(re.sub(pattern, "", text))
    return characters

def count_words():
    pattern= r"\S+" #Matches any character which is not a whitespace character [^ \t\n\r\f\v]
    words = len(re.findall(pattern, text))
    return words

def words_rankig():
    pattern = r"[\n]"
    text2 = re.sub(pattern, " ", text).lower()
    pattern1 = r"[^\w ]"#Matches Unicode word characters and " " (just whitespace)
    text3 = re.sub(pattern1, "", text2)
    pattern2= r"\S+"
    words_list = re.findall(pattern2, text3)

    frequency = []
    for i in words_list:
        frequency.append(words_list.count(i))

    aux = dict(list(zip(words_list,frequency)))
    result = [(aux[key], key) for key in aux]
    result.sort()
    result.reverse()
...
if __name__ == "__main__":
    charecters = count_characters()
    print("Number of characters: " , charecters)

    words = count_words()
    print("Number of words: " , words)

    ranking = words_rankig()
    print("Ranking:")
    print("(times,","word)")
    for x in ranking:
        print(x)
